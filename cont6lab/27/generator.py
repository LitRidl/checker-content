...
def seq_tests1(var1_bounds, var2_bounds, base=2):
    test_idx = 2
...
def seq_tests2(var1_bounds, var2_bounds, base=2):
    test_idx = 2
    for a in range(*var1_bounds):
...
def rnd_tests(test_first=57, nums=2, tests=40):
    params = {
        'word_len': [(10, 10), (10, 10)],
        'word_radix': [2, 2],
...
if __name__ == '__main__':
    if sys.argv[1] == 's' or len(sys.argv):
        # seq_tests1([0, 65], [0, 0], base=16)
        seq_tests2([0, 11], [0, 11], base=2)
    if sys.argv[1] == 'r':
        rnd_tests(test_first=2, nums=2, tests=120)
