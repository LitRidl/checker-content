/*
циклический сдвиг элементов матрицы по спирали по часовой стрелке на n элементов (n задается со стандарного потока ввода)
...
int main(void)
{
    ...
    for (int test = 0; test < tests; ++test) {
        ...
        for (int k = 0; k < shift; ++k) {
            tmp = mas[cur_size * cur_size - 1];
            for(int i = 0; i < (cur_size * cur_size - 1); ++i) {
                mas[cur_size * cur_size - 1 - i] = mas[cur_size * cur_size - 1 - i - 1];
            }
            mas[0] = tmp;
        }
