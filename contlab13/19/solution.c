/*
есть ли слово, содержащее одну гласную, возможно несколько раз
homo sapiens
affirmo
*/
#include <stdio.h>
...
int main(void)
{
    ...
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            if (letters && set_size(set_intersection(letters, get_vowels())) == 1) {
                break;
            }
            letters = EMPTY_SET;
        } else {
    ...
    if (letters != EMPTY_SET && set_size(set_intersection(letters, get_vowels())) == 1) {
        result = true;
    }
