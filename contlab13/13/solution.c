...
int main(void)
{
    ...
    Set vowels_in_words = EMPTY_SET;
    int c = 0;
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            if (set_intersection(vowels_in_words, get_vowels()) == get_vowels()) {
                break;
            }
        } else {
            if (is_vowel(c)) {
                vowels_in_words = set_insert(vowels_in_words, get_index(c));
            }
        }
    }
    if (set_intersection(vowels_in_words, get_vowels()) == get_vowels()) {
