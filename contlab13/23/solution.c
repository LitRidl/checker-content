/*
есть ли слово, все согласные которого звонкие {b, v, g, d, z}
alter ego
anathemus
*/
#include <stdio.h>
...
{
    bool result = false;
    bool word_only_voice = true, cons = false;
    ...
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            letters = EMPTY_SET;
            if (word_only_voice && cons) {
                result = true;
                break;
            }
            word_only_voice = true;
            cons = false;
        } else if (is_consonant(c) && !is_voice(c)) {
            word_only_voice = false;
        } else {
            if (is_consonant(c))
                cons = true;
    ...
    if (letters != EMPTY_SET && word_only_voice && cons) {
        result = true;
    }
