/*
есть ли слово, все согласные которого сонорные {m, n, l, h, j, r, w}
humanum errare est
primus inter pares
*/
#include <stdio.h>
...
{
    bool result = false;
    bool word_only_sonorant = true, cons = false;
    ...
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            letters = EMPTY_SET;
            if (word_only_sonorant && cons) {
                result = true;
                break;
            }
            word_only_sonorant = true;
            cons = false;
        } else if (is_consonant(c) && !is_sonorant(c)) {
            word_only_sonorant = false;
        } else {
            if (is_consonant(c))
                cons = true;
    ...
    if (letters != EMPTY_SET && word_only_sonorant && cons) {
        result = true;
    }
