/*
есть ли слово, содержащее все гласные алфавита
universalio sunt nomina
sic et non
*/
#include <stdio.h>
#include <ctype.h>
...
int main(void)
{
    ...
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            if (set_intersection(letters, get_vowels()) == get_vowels()) {
                result = true;
                break;
            }
            letters = EMPTY_SET;
    ...
    if (letters != EMPTY_SET && set_intersection(letters, get_vowels()) == get_vowels()) {
        result = true;
    }
