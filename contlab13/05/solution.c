/*
есть ли слово, содержащее только буквы латинского алфавита, имеющие графические аналоги в кириллическом алфавите {a, b, e, k, m, h, o, p, c, t, y, x}
accept
aegis zeus
*/
#include <stdio.h>
...
Set get_latin_letter()
{
    Set s = EMPTY_SET;
    char latins[] = {'a', 'b', 'e', 'k', 'm', 'h', 'o', 'p', 'c', 't', 'y', 'x'};
    for (int i = 0; i < sizeof(latins) / sizeof(latins[0]); ++i) {
        s = set_insert(s, get_index(latins[i]));
    }
    return s;
}
...
int main(void)
{
    ...
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            if (letters != EMPTY_SET && set_intersection(letters, set_complement(get_latin_letter())) == EMPTY_SET) {
                result = true;
                break;
            }
            letters = EMPTY_SET;
