/*
есть ли слово, содержащие только 5 самых частотных букв латинского алфавита {e, t, a, o, i}
tea
anathemus
*/
...
Set get_frequency_letter()
{
    Set s = EMPTY_SET;
    char latins[] = {'e', 't', 'a', 'o', 'i'};
...
int main(void)
{
    ...
    while ((c = getchar()) != EOF) {
        if (isspace(c) || iscntrl(c)) {
            if (letters != EMPTY_SET && set_intersection(letters, set_complement(get_frequency_letter())) == EMPTY_SET) {
                result = true;
                break;
            }
            letters = EMPTY_SET;
