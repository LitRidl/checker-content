...
def seq_tests2(var1_bounds, var2_bounds, base, start, fmt, same_len):
    test_idx = start
    for a in range(*var1_bounds):
        for b in range(*var2_bounds):
            with open('tests/{0:03d}.dat'.format(test_idx), 'w') as f:
                _a, _b = base_repr(a, base), base_repr(b, 2)
...
def rnd_tests(test_first, nums, tests, base, word_len, word_prefix,
             leading_zero, fmt_pair, same_len):
    params = {
        'word_len': word_len,
        'word_radix': [base, 2],
...
SEQ2_RANGE=[[0, 15 + 1], [0, 15 + 1]]

FMT_PAIR='{0} {1}'
SAME_LEN=False

RND_TESTS=400
RND_WORD_LEN=[(8, 20)] * 2  # word len in chars
RND_LEADING_ZERO=False
RND_WORD_PREFIX=[' '] * 2
        ...
        if NUMS_PER_TEST == 1:
            ...
            test_idx=seq_tests2(
                *SEQ2_RANGE, base=NUMS_RADIX, start=TEST_STARTING,
                fmt=FMT_PAIR, same_len=SAME_LEN)
