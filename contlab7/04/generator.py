    ...
    for test_idx in range(test_first, test_first + tests):
        with open('tests/{0:03d}.dat'.format(test_idx), 'w') as f:
            if nums == 1:
                ...
                f.write(generate_test2(params, fmt=fmt_pair, same_len=same_len, filter_good=filter_good))
...
SEQ1_RANGE = [[0, 100 + 1], None]
SEQ2_RANGE = [[0, 25 + 1], [0, 25 + 1]]

FILTER2_GOOD = lambda a, b: a >= b
FMT_PAIR = '{0}-{1}'
SAME_LEN = False

RND_TESTS = 200
RND_WORD_LEN = [(3, 12)] * 2  # word len in chars
